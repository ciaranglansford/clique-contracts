...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function addPool(
        ...
        require(!_isEthPool || address(_stakingToken) == address(0), "ETH pool must have zero address");
    ...
    function deposit(uint256 _pid, uint256 _amount) external payable nonReentrant {
        ...
        if (pool.isEthPool) {
            require(msg.value > 0 && _amount == 0, "ETH amount must match");
            user.amount += msg.value;
            pool.totalStaked += msg.value;
        } else {
            require(msg.value == 0, "Do not send ETH to ERC20 pool");
            if (_amount > 0) {
        ...
        emit Deposit(msg.sender, _pid, pool.isEthPool ? msg.value : _amount);
    }
